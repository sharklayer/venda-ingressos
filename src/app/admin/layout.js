...
const _navigation = [
  ...
  { name: 'Clientes', href: '/admin/clientes', icon: UsersIcon, current: false },
  { name: 'Eventos', href: '/admin/eventos', icon: FolderIcon, current: false },
  { name: 'Ingressos', href: '/admin/ingressos', icon: TicketIcon, current: false },
]
...
export default function Layout({children}) {
  ...
  if (status === "authenticated") {
    if (session?.user?.is_admin === false) {
      toast.error("Usuário não possui acesso administrativo.");
      router.replace('/');
    }
    else {
      return (
